...
class User {
  ...
  static async updateLoginTimestamp(username) {
    ...
    if (results.rows.length === 0) throw new ExpressError(`USER ${username} not found`, 404);
  }
  ...
  static async all() {
    const allUsers = await db.query(`SELECT username, first_name, last_name, phone
    FROM users`)
  ...
  static async get(username) {
    ...
    if(!user.rows[0]) throw new ExpressError(`User ${username} not found`, 404)
    return user.rows[0];
  }
  ...
  static async messagesFrom(username) {
   ...
   const results = messages.rows.map(m=>({
     id: m.id,
     body: m.body,
     sent_at: m.sent_at,
     read_at: m.read_at,
